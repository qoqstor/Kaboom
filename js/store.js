...
(() => {
  ...
  function persist(quizzes) {
    try {
      ...
      if (e && (e.name === 'QuotaExceededError' || e.code === 22)) {
        throw new Error('瀏覽器儲存空間不足：請刪除部分圖片或題庫（圖片會佔用較多空間）');
      }
      throw e;
    }
  }
  ...
  function validateImage(img, label) {
    ...
    if (!/^(data:image\/|https?:\/\/)/.test(s)) throw new Error(`${label}：圖片格式錯誤（需為上傳的圖片或 http(s) 網址）`);
    if (s.length > 700 * 1024) throw new Error(`${label}：圖片過大，請換較小的圖片`);
    return s;
  }

  // 選項可為字串（舊格式）或 {text, image}；至少要有文字或圖片其中之一
  function normalizeOption(o) {
    if (typeof o === 'string') return { text: o.trim() };
    if (o && typeof o === 'object') {
      return { text: String(o.text || '').trim(), image: o.image };
    }
    return { text: '' };
  }
  ...
  function validate(input) {
    ...
    const questions = rawQuestions.map((q, i) => {
      ...
      const image = validateImage(q.image, `第 ${i + 1} 題`);

      const options = (Array.isArray(q.options) ? q.options : [])
        .map(normalizeOption)
        .filter((o) => o.text.length > 0 || o.image)
        .map((o, j) => {
          if (o.text.length > 120) throw new Error(`第 ${i + 1} 題：選項文字過長`);
          const img = validateImage(o.image, `第 ${i + 1} 題選項 ${j + 1}`);
          const out = { text: o.text };
          if (img) out.image = img;
          return out;
        });
      if (options.length < 2) throw new Error(`第 ${i + 1} 題：至少需要 2 個選項（文字或圖片）`);
      ...
      const out = { text, options, correctIndex, timeLimit };
